...
describe('BasketComponent', () => {
  ...
  it('authentication with user', async(inject([HttpClient,HttpTestingController],
    ...
    const _body = JSON.parse('{"login":"log","password":"password"}');
    // the correct coordinates
    const body = JSON.parse('{"login":"admin","password":"password"}');
    console.log(body.login);
    userS.login(body.login, body.password).subscribe(
      (user: User) => {
        const islog = userS.isLogged;
  ...
  const app = new BasketComponent(userS);
    app.items = [
